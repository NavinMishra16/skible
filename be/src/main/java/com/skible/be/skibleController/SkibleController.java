...
public class SkibleController {
        ...
        public ResponseEntity<?> playerReady(@RequestBody Map<String, String> request) {
                ...
                if (allReady) {
                        ...
                        System.out.println("[DEBUG] Game started message sent to: /topic/room/" + roomId
                                        + "/game-started");
                ...
                try {
                        ...
                        messagingTemplate.convertAndSend(
                                        ...
                                        Map.of(
                                                        ...
                                                        "options", opts));
                        System.out.println(
                                        "[DEBUG] Manual word options sent to: /topic/room/" + roomId + "/word-options");
                } catch (Exception e) {
        ...
        public void pickWord(PickWordRequest request) {
                System.out.println(
                                "[DEBUG] Word picked: " + request.getChosenWord() + " by " + request.getPlayerName());
        ...
        public void makeGuess(GuessRequest req) {
                ...
                System.out.println("[DEBUG] After guess, current round = " + gameState.getCurrentRound());
                System.out.println("[DEBUG] Next picker: " + nextPicker);

                // 5) if we Game is completed 5 rounds the start the end game process
                if (gameState.getCurrentRound() >= 5) {
                        // a) BroadCast round 5 completion
                        messagingTemplate.convertAndSend(
                                        "/topic/room/" + req.getRoomId() + "/game-over",
                                        Map.of(
                                                        "message", "Game Over: 5 Round Complete"));
                        // b) compute the winner (non - stream)
                        Map<String, Integer> scores = gameManager.getScores(req.getRoomId());
                        ...
                        messagingTemplate.convertAndSend(
                                        "/topic/room/" + req.getRoomId() + "/winner-announcement",
                                        Map.of( 
                                                "winner",winner,
                                                "score", highest,
                                                "message","Winner: "+ winner +  " with " + "score " + highest + " points ")
                                        );

                        // c) Schedule async cleanup + final disconnect message
                        scheduler.schedule(() -> {
                                gameManager.cleanupGame(req.getRoomId());
                                messagingTemplate.convertAndSend(
                                                "/topic/room/" + req.getRoomId() + "/session-terminated",
                                                Map.of(
                                                                "message",
                                                                "Game session terminated, Thanks for playing skible!"));

                        }, 10, java.util.concurrent.TimeUnit.SECONDS);

                } 
                else {
                        // 6) announce next pick-phase
                        messagingTemplate.convertAndSend(
                        ...
                        // 7) immediately send word-options for next pick
                        try {
